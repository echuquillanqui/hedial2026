...
<div class="container-fluid logistics-page">
    ...
    <div class="logistics-panel overflow-hidden"><div class="table-responsive"><table class="table logistics-table mb-0 align-middle">
        ...
        @forelse($entries as $entry)<tr>
            ...
            <td><span class="badge {{ $entry->expiration_date->isPast() ? 'text-bg-danger' : ($entry->expiration_date->lte(today()->addDays(30)) ? 'text-bg-warning' : 'text-bg-success') }}">{{ $entry->expiration_date->format('d/m/Y') }}</span></td>
...
<div class="modal fade" id="entryModal" tabindex="-1"><div class="modal-dialog modal-lg"><form class="modal-content" method="POST" action="{{ route('warehouse.entries.store') }}">@csrf
    ...
    <div class="modal-body"><div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">Fecha de vencimiento</label><input type="date" name="expiration_date" min="{{ today()->toDateString() }}" class="form-control" required></div>
