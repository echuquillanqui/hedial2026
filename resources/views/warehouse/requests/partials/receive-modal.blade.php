          ...
          <div x-show="receiveRequestId === {{ $req->id }}">
            @foreach($req->items as $idx => $item)
              <div class="row g-2 border rounded p-2 mb-2">
                ...
                <div class="col-md-2">
                  <select class="form-select form-select-sm" name="items[{{ $idx }}][receive_status]" required>
                ...
                <div class="col-md-3">
                  <input type="text" class="form-control form-control-sm" name="items[{{ $idx }}][not_received_reason]" value="{{ $item->not_received_reason }}" placeholder="Observación de recepción">
                </div>
              </div>
            @endforeach
          </div>
