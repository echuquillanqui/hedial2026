...
<div class="container-fluid" x-data="consultationAudit()" @keydown.window="handleShortcut($event)">
    ...
    <form method="GET" class="card card-body shadow-sm mb-4 audit-consultation-filters">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-xl col-sm-4"><label class="form-label">Módulo</label><select class="form-select" name="modulo"><option value="">Todos</option>@foreach(range(1, 4) as $value)<option value="{{ $value }}" @selected((string) request('modulo') === (string) $value)>MÓDULO {{ $value }}</option>@endforeach</select></div>
