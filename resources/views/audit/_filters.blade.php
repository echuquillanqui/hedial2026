<form method="GET" class="card card-body shadow-sm mb-4 audit-filters" data-audit-filters>
    <div class="row g-2 align-items-end audit-filters__row">
        ...
        <div class="col-sm-6 col-lg-2"><label class="form-label">Módulo</label><select class="form-select" name="modulo"><option value="">Todos</option>@foreach(\App\Models\Patient::MODULES as $module)<option value="{{ $module }}" @selected((string) request('modulo') === (string) $module)>MÓDULO {{ $module }}</option>@endforeach</select></div>
