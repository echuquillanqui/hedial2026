<form method="GET" class="card card-body shadow-sm mb-4" data-audit-filters>
    <div class="row g-3 align-items-end">
        ...
        @if($showSequence ?? false)<div class="col-sm-6 col-lg-2"><label class="form-label">Secuencia</label><select class="form-select" name="secuencia"><option value="">Todas</option>@foreach(['L-M-V', 'M-J-S'] as $sequence)<option value="{{ $sequence }}" @selected(request('secuencia') === $sequence)>{{ $sequence }}</option>@endforeach</select></div>@endif
