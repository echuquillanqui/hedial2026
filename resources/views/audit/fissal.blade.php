...
<div class="container-fluid">
    ...
    <div class="card shadow-sm overflow-hidden"><div class="table-responsive"><table class="table table-sm table-striped align-middle mb-0" style="font-size:.82rem">
        <thead class="table-dark"><tr><th>Apellidos y nombres del paciente</th><th>Inicio</th><th>Final</th><th>N.° FUA</th><th>EPO 2000</th><th>EPO 4000</th><th>Vit. B12</th><th>Hierro</th><th>Calcitriol</th><th>Fecha</th><th>Lic. inicia</th><th>Lic. finaliza</th><th>Nefrólogo</th></tr></thead>
        <tbody>@forelse($orders as $order)@php($nurse = $order->nurse)@php($medical = $order->medical)<tr>
            ...
            <td class="text-nowrap">{{ optional($order->fecha_orden)->format('d/m/Y') }}</td><td>{{ $nurse?->enfermeroInicia?->name ?: '—' }}</td><td>{{ $nurse?->enfermeroFinaliza?->name ?: '—' }}</td><td>{{ $medical?->usuarioInicia?->name ?: $medical?->usuarioFinaliza?->name ?: '—' }}</td>
        </tr>@empty<tr><td colspan="13" class="text-center text-muted py-5">No hay datos para los filtros seleccionados.</td></tr>@endforelse</tbody>
