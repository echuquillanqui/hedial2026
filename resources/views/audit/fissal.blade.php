...
<div class="container-fluid">
    ...
    <div class="card shadow-sm overflow-hidden"><div class="table-responsive"><table class="table table-sm table-striped align-middle mb-0" style="font-size:.82rem">
        ...
        <tbody>@forelse($orders as $order)@php($nurse = $order->nurse)@php($medical = $order->medical)<tr>
            <td class="text-nowrap"><strong>{{ $order->patient?->full_name }}</strong></td><td class="text-nowrap">{{ $order->patient?->secuencia ?: '—' }}</td><td>{{ optional($order->treatments->first())->hora ? substr($order->treatments->first()->hora, 0, 5) : '—' }}</td><td>{{ optional($order->treatments->last())->hora ? substr($order->treatments->last()->hora, 0, 5) : '—' }}</td>
