...
<div class="container py-3">
    ...
    <form method="POST" action="{{ route('medication-catalog.update') }}">@csrf @method('PUT')
        <div class="card border-0 shadow-sm"><div class="table-responsive"><table class="table table-sm align-middle mb-0">
            <thead class="table-light"><tr><th class="ps-3">Código FUA</th><th>Producto farmacéutico</th><th>Cantidad referencial</th><th class="pe-3">Frecuencia</th></tr></thead>
            <tbody>@foreach($medications as $medication)<tr>
                ...
                <td class="pe-3" style="min-width:140px"><input required class="form-control form-control-sm" name="medications[{{ $medication->id }}][frequency]" value="{{ old("medications.{$medication->id}.frequency", $medication->frequency) }}"></td>
            </tr>@endforeach</tbody>
        </table></div></div>
