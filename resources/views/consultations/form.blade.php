    ...
    <form method="POST" action="{{ $editing ? route('consultations.update', $consultation) : route('consultations.store') }}">@csrf @if($editing)@method('PUT')@endif
        ...
        <section id="anamnesis" class="clinical-panel active">
            ...
            <div class="card section-card shadow-sm mb-3"><div class="card-body"><h5 class="section-title mb-3">Evaluación clínica</h5><div class="row g-3">
                <div class="col-md-4"><label class="form-label">Fecha de inicio de diálisis</label><input type="date" name="dialysis_start_date" class="form-control" value="{{ old('dialysis_start_date', optional($consultation->dialysis_start_date)->format('Y-m-d')) }}"></div>
                @foreach(['disease_duration'=>'Tiempo de enfermedad','etiology'=>'Etiología','vascular_access'=>'Acceso vascular actual','symptoms'=>'Signos y síntomas'] as $field=>$label)<div class="col-md-4"><label class="form-label">{{ $label }}</label><input name="{{ $field }}" class="form-control" value="{{ old($field, $consultation->$field) }}"></div>@endforeach
                @foreach(['reason'=>'Motivo de consulta','current_illness'=>'Enfermedad actual / anamnesis','history'=>'Antecedentes','physical_exam'=>'Estado general / examen físico','lung_exam'=>'Tórax y pulmones','cardiac_exam'=>'Examen cardiovascular','treatment_plan'=>'Plan / indicaciones','observations'=>'Observaciones'] as $field=>$label)<div class="col-md-6"><label class="form-label">{{ $label }}</label><textarea name="{{ $field }}" rows="3" class="form-control">{{ old($field, $consultation->$field) }}</textarea></div>@endforeach
            </div></div></div>
            <div class="card section-card shadow-sm mb-3"><div class="card-body"><h5 class="section-title mb-3">Prescripción de diálisis y tratamiento complementario</h5><div class="row g-3">
                @foreach(['dialysis_prescription'=>'Prescripción de diálisis','dialysis_hours'=>'Tiempo (horas)','filter_area'=>'Área de filtro (m²)','hemoglobin'=>'Hemoglobina (mg/dl)','epoetin_dose'=>'Epoetina alfa 2000 UI','hydroxocobalamin_dose'=>'Hidroxocobalamina 1 mg','iron_dose'=>'Hierro 100 mg'] as $field=>$label)<div class="col-md-4"><label class="form-label">{{ $label }}</label><input name="{{ $field }}" type="{{ in_array($field, ['dialysis_hours','filter_area','hemoglobin']) ? 'number' : 'text' }}" step="0.1" class="form-control" value="{{ old($field, $consultation->$field) }}"></div>@endforeach
                @foreach(['anemia_treatment'=>'Anemia','bone_mineral_treatment'=>'Alteración metabolismo óseo mineral','antihypertensive_treatment'=>'Antihipertensivos'] as $field=>$label)<div class="col-md-4"><label class="form-label d-block">{{ $label }}</label><div class="btn-group" role="group"><input class="btn-check" type="radio" name="{{ $field }}" id="{{ $field }}_yes" value="1" @checked((string) old($field, $consultation->$field) === '1')><label class="btn btn-outline-success" for="{{ $field }}_yes">Sí</label><input class="btn-check" type="radio" name="{{ $field }}" id="{{ $field }}_no" value="0" @checked((string) old($field, $consultation->$field) === '0')><label class="btn btn-outline-secondary" for="{{ $field }}_no">No</label></div></div>@endforeach
                <div class="col-12"><label class="form-label">Otros tratamientos</label><textarea name="other_treatment" rows="2" class="form-control">{{ old('other_treatment', $consultation->other_treatment) }}</textarea></div>
            </div></div></div>
        ...
        <section id="recipe" class="clinical-panel">
            <div class="card section-card shadow-sm mb-3"><div class="card-header bg-white py-3 d-flex justify-content-between"><strong class="section-title"><i class="bi bi-prescription2 me-1"></i> Receta mensual</strong><button type="button" id="addMedication" class="btn btn-sm btn-primary">Agregar medicamento</button></div><div class="table-responsive"><table class="table align-middle mb-0"><thead class="table-light"><tr><th>Código</th><th style="min-width:260px">Medicamento</th><th style="min-width:240px">Prescripción</th><th>C. prescrita</th><th>C. entregada</th><th></th></tr></thead><tbody id="medications">
                @foreach(old('medications', $medications->toArray()) as $i=>$medication)<tr class="medication-row"><td class="position-relative"><input autocomplete="off" class="form-control medication-search medication-code" name="medications[{{ $i }}][fua_code]" value="{{ $medication['fua_code'] ?? '' }}"><div class="medication-results d-none"></div></td><td class="position-relative"><input autocomplete="off" required class="form-control medication-search medication-name" name="medications[{{ $i }}][description]" value="{{ $medication['description'] ?? '' }}"><div class="medication-results d-none"></div></td><td><input class="form-control" name="medications[{{ $i }}][c]" value="{{ $medication['c'] ?? '' }}"></td><td><input type="number" min="0" step="0.01" class="form-control prescribed-quantity" name="medications[{{ $i }}][prescribed_quantity]" value="{{ $medication['prescribed_quantity'] ?? 0 }}"></td><td><input type="number" min="0" step="0.01" class="form-control delivered-quantity" name="medications[{{ $i }}][delivered_quantity]" value="{{ $medication['delivered_quantity'] ?? 0 }}"></td><td><button type="button" class="btn btn-outline-danger remove-row">×</button></td></tr>@endforeach
            </tbody></table></div></div>
        </section>
        ...
        <div class="d-flex gap-2 justify-content-end sticky-bottom bg-light py-3"><button class="btn btn-success btn-lg px-4"><i class="bi bi-check2-circle me-1"></i> Guardar consulta</button>@if($editing)<a target="_blank" href="{{ route('consultations.pdf', $consultation) }}" class="btn btn-outline-danger btn-lg">Generar consulta PDF</a><a target="_blank" href="{{ route('consultations.prescription.pdf', $consultation) }}" class="btn btn-danger btn-lg">Generar receta PDF</a>@endif</div>
    </form>
</div>
@endsection
@push('scripts')<script>
document.addEventListener('DOMContentLoaded', () => {
    ...
    let medicationTimer;
    meds.addEventListener('input',e=>{if(!e.target.classList.contains('medication-search'))return;const results=e.target.parentElement.querySelector('.medication-results'),term=e.target.value.trim();clearTimeout(medicationTimer);if(term.length<1){results.classList.add('d-none');return;}medicationTimer=setTimeout(async()=>{const response=await fetch(`{{ route('medication-catalog.search') }}?q=${encodeURIComponent(term)}`);const items=await response.json();results.innerHTML=items.map(x=>`<div class="cie-option medication-option" data-code="${escapeHtml(x.code||'')}" data-name="${escapeHtml(x.name)}" data-quantity="${x.reference_quantity}"><strong>${escapeHtml(x.code||'Sin código')}</strong> — ${escapeHtml(x.name)} <small class="text-muted">(${x.reference_quantity} · ${escapeHtml(x.frequency)})</small></div>`).join('')||'<div class="p-3 text-muted">Sin resultados</div>';results.classList.remove('d-none');},200);});
    meds.addEventListener('click',e=>{const option=e.target.closest('.medication-option');if(!option)return;const row=option.closest('.medication-row');row.querySelector('.medication-code').value=option.dataset.code;row.querySelector('.medication-name').value=option.dataset.name;row.querySelector('.prescribed-quantity').value=option.dataset.quantity;row.querySelector('.delivered-quantity').value=option.dataset.quantity;row.querySelectorAll('.medication-results').forEach(x=>x.classList.add('d-none'));});
    function medicationRow(i){return `<tr class="medication-row"><td class="position-relative"><input autocomplete="off" class="form-control medication-search medication-code" name="medications[${i}][fua_code]"><div class="medication-results d-none"></div></td><td class="position-relative"><input autocomplete="off" required class="form-control medication-search medication-name" name="medications[${i}][description]"><div class="medication-results d-none"></div></td><td><input class="form-control" name="medications[${i}][c]"></td><td><input type="number" min="0" step=".01" value="0" class="form-control prescribed-quantity" name="medications[${i}][prescribed_quantity]"></td><td><input type="number" min="0" step=".01" value="0" class="form-control delivered-quantity" name="medications[${i}][delivered_quantity]"></td><td><button type="button" class="btn btn-outline-danger remove-row">×</button></td></tr>`;}
