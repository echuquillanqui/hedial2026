    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body bg-light">
            <form method="GET" action="{{ route('orders.multisectorial.create') }}" class="row g-2 align-items-end">
                ...
                <div class="col-lg-2"><label class="data-title">Módulo</label><select name="modulo" class="form-select border-success"><option value="">Todos</option>@foreach(\App\Models\Patient::MODULES as $value)<option value="{{ $value }}" @selected((string)request('modulo')===(string)$value)>{{ $value === \App\Models\Patient::ISOLATED_MODULE ? 'Módulo AISLADO' : 'Módulo '.$value }}</option>@endforeach</select></div>
