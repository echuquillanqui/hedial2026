...
@endif
<h1>ANEXO N.° 12</h1><h1>REGISTRO DIARIO DE ATENCIONES DE ENFERMERÍA Y COMPLICACIONES DURANTE LA<br>SESIÓN DE HEMODIÁLISIS</h1><div>(llenar por módulo y por día)</div></div>
...
<tr><td class="procedure">{{ $procedure === $previous ? '' : $procedure }}</td><td class="detail">{{ $detail }}</td><td class="quantity">
@foreach(['1','2','3','4'] as $shift)
@if(data_get($annex->values, "$key.shifts.$shift", 0) > 0)
T{{ $shift }}: {{ data_get($annex->values, "$key.shifts.$shift") }}
@if(!$loop->last)
<br>
@endif
@endif
@endforeach
@if(data_get($annex->values, "$key.quantity", 0) === 0)
0
@endif
</td><td>{{ data_get($annex->values, "$key.observations") }}</td></tr>
